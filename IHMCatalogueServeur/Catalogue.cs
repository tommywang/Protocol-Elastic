...
namespace IHMCatalogueServeur {
    public partial class Catalogue : Form {
        ...
        private void handleInterfaceEvents(object sender, ServerToolsEvent e) {
            switch (e.Type) {
                case ServerToolsEvent.typeEvent.MESSAGE:
                    ...
                    else AppendTextToLog("Client number " + e.Id + " sent an empty message!" + Environment.NewLine);
                    break;
        ...
        private void BroadCastExctinction() {
            mySelf.Broadcast(DataUtils.EXTINCTION_CODE);
        }

        private void CheckIfStillHere(int id) {
            mySelf.Send(catalogue_name, "", "operation", "0", new List<string>() { DataUtils.MANIFEST_PRESENCE_CODE }, id);
